...
- [x] Identidad «Marcador» Fase 6 (superficie publica + emails + assets): landing y blog reescritos a Marcador de forma INCONDICIONAL (decision owner: la cara publica NO depende del flag; app interna sigue flag-gated). (public)/layout.tsx aplica `.theme-marcador` + fuentes Archivo/Instrument/JetBrains Mono en el subarbol; `.dark .theme-marcador:not(.marketing-marcador)` fuerza claro en marketing y habilita islas oscuras anidando `.dark`. 10 componentes de marketing reescritos con tokens semanticos (Hero con tarjeta KPIs + mini-grid, PainPoints/footer/destacado blog como islas tinta, Pricing con Pro invertido + comparativa, Features mono 01-08, claves i18n eyebrow/cell* nuevas en es/en). Blog desde DB con chips de categoria por color semantico + PlaceholderFoto rayado (ignora imageUrl stock por brief-fotografia.md; detalle text-first). Emails: rama flag-on de plantillaEmail plana (header papel+isotipo, boton solido) + moduloMarcador (tarjeta pista/fecha/hora, borde 2px tinta) en confirmacion; legacy intacto. Assets: SVGs marca en public/brand, icon.svg + iconos PWA regenerados (verde), manifest theme_color #157A54. Ratchet de color 518->433. Verificado: typecheck, lint, 567 tests, build, screenshots landing/blog.
- [x] Identidad «Marcador» Fase 5 (resto, mismo flag): neutralizacion CSS del legado (orbes/shimmer/glassmorphism/gradientes apagados bajo .theme-marcador sin tocar componentes), src/lib/avatar.ts (ui-avatars sobre verde marca, retirado #6366f1), IngresosSemana con trama SVG en pendiente, CourtUtilization con tokens de estado, badge socio inactivo neutro+icono, emails con EMAIL_BRAND verde/papel/tinta + isotipo nuevo. Pendiente post-gate: retirar flag+tema antiguo, migrar 518 usos crudos, flujos 2b/2d/3b/3e aplazados (ver docs/identidad-marcador.md)
- [x] Identidad «Marcador» Fase 4 (flujos criticos, mismo flag): src/lib/tenant-theme.ts (escala OKLCH 50-900, on-primary con AA garantizado, gate de 12 tenants extremos en tenant-theme.test.ts), vars --tenant-* en ClubLayout (+body para portales Radix), .btn-tenant en CTAs del jugador, PreviewTenant en Ajustes con check de contraste, GridReservas 2b (44px, chips de dia, tramas) y CourtGridView 3b (borde izq 3px por estado + icono). Pendiente: barra resumen sticky, drawer 392px, rail 72px, pantallas 2d
- [x] Identidad «Marcador» Fase 3 (componentes core, mismo flag): button/input/textarea/select/badge/table/toast adaptados por rama cva (foco outline verde, disabled 45%, press 120ms, borde 1.5px border-strong, badges de estado con foreground AA, tabla densa 40px) + nuevo ui/kpi.tsx; tokens --primary-hover, --border-strong, --status-*-foreground
- [x] Identidad «Marcador» Fase 2 (shell, mismo flag): `NavItem.group` + `agruparNavItems()` (6 grupos, orden del prototipo), Sidebar/MobileSidebar tinta agrupadas con barra activa 3px, Header admin 56px, bottom-navs sobre surface-raised con safe-area, ClubLayout sin gradientes/blur + «Powered by», LogoIcon con isotipo nuevo (claseRelleno). Claves nav.grupo* en es/en
- [x] Identidad «Marcador» Fase 1 (foundations, flag `NEXT_PUBLIC_TEMA_MARCADOR`): tokens nuevos en globals.css bajo `.theme-marcador` (papel/tinta, verde pista, sidebar tinta, status, dataviz, motion), fuentes Archivo + Instrument Sans (next/font, reutilizan `--font-inter`/`--font-sora`), colores semanticos nuevos en tailwind.config.ts (info/success/warning/error, surface-raised, dataviz-1..6), lint ratchet anti colores crudos (`scripts/lint-colores-crudos.mjs` + baseline, encadenado a `npm run lint`). Flag apagado = cero cambio visual. Fases 2-5 pendientes: ver `docs/identidad-marcador.md` y `design_handoff_identidad_marcador/`
...
- [x] Excelencia de producto (Bloque 3): docs/entregabilidad-email.md (checklist SPF/DKIM/DMARC en Resend + mail-tester para el owner) y reply-to del club en emails de broadcast; "Repetir reserva" en historial del jugador (boton RotateCcw -> /club/[slug]/reservar?fecha=YYYY-MM-DD validado contra maxAdvanceBooking, src/lib/fechas.ts con proximaFechaMismoDiaSemana + 14 tests); scripts/normalizar-tier-legacy.ts (dry-run por defecto, --apply, --confirm si DB remota) ejecutado en DB dev (19 clubes essential->starter) y default del schema a "starter" — PENDIENTE ejecutarlo en PROD; pass Lighthouse (club Perf 63->71 con banner next/image priority, A11y landing 88->100 y club 90->96, SEO 100; zoom habilitado quitando maximumScale) y pricing de la landing alineado con los limites reales del codigo
...
**Sesion Stripe E2E TEST - 22/07/2026** `[x]`
- [x] Stripe CLI local + webhook real; catalogo TEST Starter 19 EUR, Pro 49 EUR y Enterprise 99 EUR
- [x] SaaS real: Starter, fin de trial, upgrade a Pro, cancelacion, bloqueo, recontratacion y `trial_will_end`
