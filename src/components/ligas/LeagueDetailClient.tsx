// src/components/ligas/LeagueDetailClient.tsx
'use client';

import React, { useState, useEffect } from 'react';
import { League, Team, User, Match } from '@prisma/client';
import { PlusCircle, Zap, Loader2, Pencil, Save, Edit, XCircle } from 'lucide-react';
...
const LeagueDetailClient: React.FC<LeagueDetailClientProps> = ({ league, users }) => {
  ...
  // NUEVO: Agrupar partidos por jornada
  const groupedMatches = league.matches.reduce((acc, match) => {
    const round = match.round || 0; // Agrupar partidos sin jornada en '0'
    if (!acc[round]) {
      acc[round] = [];
    }
  ...
  return (
    <>
      ...
      {/* RESTAURADO: Columna de lista de equipos */}
      ...
      <div className="lg:col-span-2 space-y-8">
        <div className="bg-gray-800 p-6 rounded-xl shadow-lg">
          ...
          {league.matches.length > 0 ? (
            <div className="space-y-6">
              {/* NUEVO: Iterar sobre jornadas agrupadas */}
              {Object.entries(groupedMatches).sort(([a], [b]) => Number(a) - Number(b)).map(([jornada, matches]) => (
                <div key={jornada}>
                  <h3 className="text-lg font-semibold text-indigo-400 mb-2 border-b border-gray-700 pb-1">Jornada {jornada}</h3>
                  <ul className="divide-y divide-gray-700">
                    {matches.map(match => (
              ...
              {/* RESTAURADO: Botón para guardar fechas */}
        ...
        {/* Tabla de clasificación (sin cambios) */}
        <div className="bg-gray-800 p-6 rounded-xl shadow-lg">
