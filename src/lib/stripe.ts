...
// Cliente Stripe singleton con lazy initialization
// Evita errores en build cuando STRIPE_SECRET_KEY no esta definida
...
export const PLAN_PRICES = {
  starter: {
    get monthly() { return process.env.STRIPE_PRICE_STARTER_MONTHLY ?? "" },
    name: "Starter",
    price: 19,
    limits: {
      courts: 4,
  ...
  pro: {
    get monthly() { return process.env.STRIPE_PRICE_PRO_MONTHLY ?? "" },
    name: "Pro",
    price: 49,
    limits: {
      courts: -1, // ilimitado
  ...
  enterprise: {
    get monthly() { return process.env.STRIPE_PRICE_ENTERPRISE_MONTHLY ?? "" },
    name: "Enterprise",
    price: 99,
    limits: {
      courts: -1,
